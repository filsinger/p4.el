...
and two version designators specifying which versions to compare."
  ...
  (if historic
	...
	(if (= 0 (buffer-size))
	  ...
	  (goto-char (point-min))
	  (shrink-window-if-larger-than-buffer)
	  (vc-activate-diff-buffer "*vc-diff*")))
      (not unchanged))))
...
(make-face 'vc-diff-file-face)
;;(set-face-foreground 'vc-diff-file-face "black")
(set-face-background 'vc-diff-file-face "gray90")

(make-face 'vc-diff-head-face)
;;(set-face-foreground 'vc-diff-head-face "black")
(set-face-background 'vc-diff-head-face "gray95")

(make-face 'vc-diff-ins-face)
(set-face-foreground 'vc-diff-ins-face "blue")

(make-face 'vc-diff-del-face)
(set-face-foreground 'vc-diff-del-face "red")

(make-face 'vc-diff-change-face)
(set-face-foreground 'vc-diff-change-face "dark green")

(defun vc-set-extent-property (start end property value)
  (if (featurep 'xemacs)
...
(defun vc-buffer-set-face-property (regexp face-property)
  (save-excursion
    (goto-char (point-min))
    (while (re-search-forward regexp nil t)
      (let ((start (match-beginning 0))
	    (end (match-end 0)))
	(vc-set-extent-property start end
				'face face-property)))))

(defun vc-activate-diff-buffer (buffer-name)
  (save-excursion
    (set-buffer buffer-name)
    (setq buffer-read-only nil)
    (if vc-colorized-diffs
	(progn
	  (vc-buffer-set-face-property "^=.*\n" 'vc-diff-file-face)
	  (vc-buffer-set-face-property "^\\(@\\|\\*\\).*" 'vc-diff-head-face)
	  (vc-buffer-set-face-property "^\\(\\+\\|>\\).*$" 'vc-diff-ins-face)
	  (vc-buffer-set-face-property "^\\(\\-\\|<\\).*$" 'vc-diff-del-face)
	  (vc-buffer-set-face-property "^\\(---.*\\)\n" 'vc-diff-file-face)
	  (vc-buffer-set-face-property "^\\(--- .* ----\\)\n"
				       'vc-diff-head-face)
	  (vc-buffer-set-face-property "^!.*$" 'vc-diff-change-face)))
    ...
    (while (re-search-forward "^\\(==== //\\).*\n\\(\\(\n\\|[^=\n].*\n\\)*\\)"
      ...
      (let ((fname (get-char-property (match-end 1) 'filename))
	    ...
	    (end (match-end 2)))
	(vc-set-extent-property start end 'local-fname fname)))
    ...
    (while (re-search-forward
      ...
      (let ((first-line (string-to-int (match-string 2)))
	    ...
	    (end (match-end 3)))
	(vc-set-extent-property start end 'first-line first-line)
	(vc-set-extent-property start end 'start start)))

    (goto-char (point-min))
    (use-local-map vc-diff-map)
    (setq buffer-read-only t)))

(defun vc-scroll-down-1-line ()
  "Scroll down one line"
  (interactive)
  (scroll-down 1))

(defun vc-scroll-up-1-line ()
  "Scroll up one line"
  (interactive)
  (scroll-up 1))

(defun vc-scroll-down-1-window ()
  "Scroll down one window"
  (interactive)
  (scroll-down
   (- (window-height) next-screen-context-lines)))

(defun vc-scroll-up-1-window ()
  "Scroll up one window"
  (interactive)
  (scroll-up
   (- (window-height) next-screen-context-lines)))

(defun vc-top-of-buffer ()
  "Top of buffer"
  (interactive)
  (goto-char (point-min)))

(defun vc-bottom-of-buffer ()
  "Bottom of buffer"
  (interactive)
  (goto-char (point-max)))

(defun vc-delete-other-windows ()
  "Make buffer full height"
  (interactive)
  (delete-other-windows))

(defun vc-goto-next-file-in-diff ()
  "Next diff"
  (interactive)
...
(defun vc-goto-prev-file-in-diff ()
  "Previous diff"
  (interactive)
  (if (= (point) (point-min))
      (error "At top"))
...
(defun vc-make-derived-map (base-map)
  (let (map)
    (if (featurep 'xemacs)
	(progn
...
(defvar vc-basic-map
  (let ((map (make-sparse-keymap)))
    (if  (featurep 'xemacs)
	(define-key map [button2] 'vc-buffer-mouse-clicked)
      (define-key map [mouse-2] 'vc-buffer-mouse-clicked))
    (define-key map [return]  'vc-buffer-commands)
    (define-key map "k"	 'vc-scroll-down-1-line)
    (define-key map "j"	 'vc-scroll-up-1-line)
    (define-key map "b"	 'vc-scroll-down-1-window)
    (define-key map [backspace] 'vc-scroll-down-1-window)
    (define-key map " "	 'vc-scroll-up-1-window)
    (define-key map "<"	 'vc-top-of-buffer)
    (define-key map ">"	 'vc-bottom-of-buffer)
    (define-key map "="	 'vc-delete-other-windows)
    map))

(defvar vc-diff-map
  (let ((map (vc-make-derived-map vc-basic-map)))
    (define-key map "d"	 'vc-goto-next-diff)
    (define-key map "u"	 'vc-goto-prev-diff)
    (define-key map "n"	 'vc-goto-next-file-in-diff)
    (define-key map "p"	 'vc-goto-prev-file-in-diff)
